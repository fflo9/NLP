...
#convert the dictionaries to a hf dataset then datasetdict
trainDataset = Dataset.from_dict(train_dict)
devDataset = Dataset.from_dict(dev_dict)

datasetDict = DatasetDict({'train': trainDataset, 'dev': devDataset})
tokenizedData = datasetDict.map(tokenize_and_align_labels, batched=True)
print('Columns in the new dataset:', tokenizedData['train'].column_names)

#example result
print(tokenizedData['train'][1])
print(tokenizedData['dev'][5])
...
#initialize trainer with weighted classes
class WeightedTrainer(Trainer):
  def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
    ...
    weights = torch.tensor([0.5, 2.0, 2.0, 1.2, 1.2, 2.5, 2.5])
...
#get dev predictions
output = trainer.predict(dev_final)
...
def compute_metrics(output, predictions):
...
print(compute_metrics(output, predictions))

#save the results
with open('/content/roberta_predictions.iob2', 'w', encoding='utf-8') as file:
    for i, sentence_data in enumerate(dev_data):
        original_words = sentence_data[0]
        word_ids = tokenizedData['dev'][i]['input_ids']
        preds = predictions[i]
